...
public class MessageService {
    ...
    public Message createMessage(MessageRequest messageRequest, String userId) {
       ...
       //     messageToCreate.setIsReceived(isReceived);
          ...
          //  roomToCreate.setIsReceived(Boolean.FALSE);
            ...
            if(!messageRepository.findByReceiver(userIdx).isEmpty()){
                log.info("메세지 시작");
                if(messageToCreate.getSender().getUserIdx().equals(messageRepository.findByReceiver(userIdx).get(0).getRoom())&&!userIdx.equals(messageToCreate.getRoom()))
                {
    ...
    public List<MessageResponse> readMyMessage(String userId){
        ...
        messages.forEach(room -> {
            if(room.getSender().getUserIdx().equals(userIdx)||room.getReceiver().equals(userIdx)) {
                ...
                List<Message> messageList = messageRepository.findByRoom1(friendId);
    ...
    public List<SpecificMessageReponse> readMessage(String userId, Long friendId){
        ...
        List<Message> messages = messageRepository.findByRoom(friendId);
